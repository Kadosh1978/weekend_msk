...
route, route_created = Route.objects.update_or_create(
    ...
    defaults={
        ...
        "content": (
            "Маршрут рассчитан на два дня: первый день — для Рязанского кремля, центра и главных городских символов, второй — для музеев, неспешных прогулок и более глубокого знакомства с городом.\n\n"
            "Почему стоит поехать.\n"
            "Два дня в Рязани позволяют не ограничиваться только кремлём. Можно спокойно пройти центр, добавить музеи, провести вечер без спешки и увидеть город не только как точку на один день, а как полноценное направление для короткой поездки.\n\n"
