...
route, route_created = Route.objects.update_or_create(
    ...
    defaults={
        ...
        "content": (
            "Маршрут рассчитан на очень плотный день в Казани: он подойдёт, если времени мало, но хочется увидеть кремль, центр, набережную и несколько самых узнаваемых точек города.\n\n"
            "Почему стоит поехать.\n"
            "Казань за один день — это формат максимальной программы. Он не заменит полноценную поездку на два-три дня, но поможет быстро почувствовать город: увидеть главные символы, пройтись по центру, попробовать татарскую кухню и понять, куда вернуться подробнее.\n\n"
